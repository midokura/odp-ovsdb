                                             ...
                                             CommandProvider
{
    ...
    private VtepConfService vtepConfService = new VtepConfService();
    ...
    public StatusWithUuid insertRow(Node node, String tableName, String parent_uuid, Table<?> row) {
        ...
        if (row.getTableName().getName().equalsIgnoreCase("Bridge")) {
            ...
            statusWithUUID = vtepConfService.insertRow(node, parent_uuid, row);
        }
    ...
    public void _vteptest(CommandInterpreter ci) {
        ...
        pp.setVlan_bindings(vlanBindings);
        st = insertRow(node, Physical_Port.NAME.getName(), lsId.toString(), pp);
        if (!st.isSuccess()) {
            ci.println("NOES! failed.." + st.getCode());
            return;
        }
        ...
        ci.println("The Ucast_Macs_Remote is " + st.getUuid());
    }
    ...
    /*
     * This is a helper to the ConfigurationService that packs all stuff related
     * to VTEP. Needs to be inner class because requires private methods from
     * ConfigurationService, but trying to group vtep-specific stuff here.
     *
     * Note that all this static code will be removed with the schema based
     * approach post-Hydrogen.
     */
    private class VtepConfService {

        private VtepConfService() {}

        // TODO clean, constants, etc.
        ...
        /**
         * Corresponds to ovs-vsctl add-br <name>.
         *
         * Use ovsdb-client dump unix:/var/run/openvswitch/db.sock hardware_vtep
         ...
         * of operations below.
         */
        private StatusWithUuid doInsertTransact(Node node, String tableName,
        ...
        private StatusWithUuid insLogicalBindingStats(Node node,
                                                      String parentUuid,
                                                      Table<?> row) {
            return null;
        }
